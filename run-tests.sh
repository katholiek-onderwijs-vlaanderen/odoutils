...
while true
do
	hash=$(find . -type f -exec ls -l {} + | sort | md5sum)
	...
	then
		...
		lines=`expr $(tput lines) - 10 - $error_count`
	...
	hash2=$(find . -type f -exec ls -l {} + | sort | md5sum)
